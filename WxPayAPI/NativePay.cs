...
{
    partial class NativePay
    {
        private readonly AppConfig _config;
        public NativePay(AppConfig config)
        {
            _config = config;
        }
       ...
       */
        ...
        {
            LogManager.Log(this.GetType().ToString(), "Native pay mode 1 url is producing...");

            WxPayData data = new WxPayData(_config.AppId);
            data.SetValue("appid", _config.AppId);//公众帐号id
            data.SetValue("mch_id", _config.Mchid);//商户号
            ...
            LogManager.Log(this.GetType().ToString(), "Get native pay mode 1 url : " + url);
            return url;
        }
